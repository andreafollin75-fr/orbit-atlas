...
async function loadWorkflow(id: string) {
  ...
  setWorkflow(data);
  if (data.sources) setSources(JSON.parse(data.sources));
  if (data.outline) setOutline(JSON.parse(data.outline));
  if (data.images) setImages(JSON.parse(data.images));
  return data;
}
...
return (
  ...
  {workflow && workflow.status === "RENDERING" && (
    <section className="space-y-3 border-t pt-6">
    <p className="text-sm text-[#857f74]">Toutes les etapes IA sont terminees. Rendu Figma a venir (node 8).</p>
    </section>
    )}
